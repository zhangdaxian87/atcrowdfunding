...
public class CrowdUtil {
	 ...
	 * @param sign 签名编号
	 * @param skin 模板编号
	 * @return 返回调用结果是否成功 成功：返回验证码 失败：返回失败消息 状态码: 200 正常；400 URL无效；401 appCode错误； 403 次数用完； 500 API网管错误
	 */
	...
	/**
	 * 给远程第三方短信接口发送请求把验证码发送到用户手机上(模拟操作)
	 * 
	 * @param host 短信接口调用的URL地址
	 * @param path 具体发送短信功能的地址
	 * @param method 请求方式
...
//			return ResultEntity.failed(reasonPhrase);
			return ResultEntity.failed("发送验证码");
